...
### 🥇 黄金行情窗口
- **固定黄金标的**：内置伦敦金现、美元汇率、伦敦金现（RMB/g）、纽约金、AU9999
- **分标的独立窗口**：每个黄金标的独立悬浮窗口，可分别开启/关闭
- **显示可调**：支持字体、透明度、是否显示代码、刷新频率、置顶等设置
- **分标的定时显示**：可为不同黄金标的设置各自的自动显示时间段
...
- **钉钉通知**：支持钉钉机器人推送交易信号
- **黄金价格预警**：支持按价格阈值触发黄金预警，并可配置重复提醒频率
...
- **监控标的开关**：可分别控制伦敦金现、美元汇率、伦敦金现（RMB/g）、纽约金、AU9999 的窗口显示。
- **显示设置**：
  - **字体与透明度**：调整字体大小、背景透明度和文字透明度。
  - **刷新间隔**：设置黄金行情刷新频率（秒）。
  - **显示选项**：可选择是否显示代码、是否始终置顶。
- **定时显示**：可按黄金标的分别设置多个显示时间段，启用后仅在设定时间段内显示对应窗口。
...
- **黄金行情预警**：
  - **扫描间隔**：设置黄金预警扫描间隔（秒）。
  - **任务列表**：为黄金标的设置预警价格、预警频率（分钟）和开关状态。
  - **触发规则**：当当前价格 >= 预警价格时触发通知。
  - **频率说明**：预警频率为 0 时仅提醒一次并自动关闭该任务；大于 0 时在对应分钟后自动恢复监控并可再次触发。
  - **运行时段**：黄金预警不依赖 A 股交易时段，非交易时段也会按配置继续扫描。
- **策略控制**：
...
- 行情窗口股票列表和显示设置
- 黄金窗口标的开关、显示样式与分标的定时显示配置（gold_window）
- A 股预警任务、黄金预警任务与对应扫描间隔（alert.tasks / alert.gold_tasks）
- 钉钉通知配置
...
### Q: 黄金预警如何配置？
A: 在设置窗口的"行情预警"标签页中找到"黄金行情预警"部分，必须先配置钉钉机器人的 Webhook 和 Secret，然后添加预警任务并设置价格阈值。

### Q: 黄金预警和股票预警可以同时运行吗？
A: 可以。两个预警系统独立运行，使用同一个钉钉通知配置。支持分别配置扫描间隔。

### Q: 定时显示不工作？
A: 确保在对应标题页中启用了"定时显示"，并检查时间段配置是否正确。时间格式必须为 `HH:MM` (24小时制)。
